...
class HistoricalMapping:
    ...
    def liquidation_map_depth_from_historical(
        ...
        for df_l, label in zip(df_losscut_list, labels):
            df_l.to_csv(
                # f"./{path}/{save_title.replace('.png', '')}_{label.replace(' ', '_')}_buy.csv"y
                f"./{path}/{save_title.replace('.png', '')}_{label.replace(' ', '_')}_buy.csv"
            )

        # Save current price to file
        current_price_file = f"./{path}/{save_title.replace('.png', '')}_current_price.txt"
        with open(current_price_file, "w") as f:
...
#         plt.close()
        ...
        for df_l, label in zip(df_losscut_list, labels):
            df_l.to_csv(
#                 f"{save_title.replace('.png', '')}_{label.replace(' ', '_')}_sell.csv"
                f"./{path}/{save_title.replace('.png', '')}_{label.replace(' ', '_')}_sell.csv"
            )
#         Save current price to a file
